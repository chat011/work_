...
def post_process_scraped_data(result):
    """
    Post-process scraped data to fix image URLs and improve data quality
...
async def upload_products(request: UploadProductsRequest):
    ...
    try:
        ...
        upload_file = os.path.join(logs_dir, f"uploaded_products_{timestamp}.json")
        with open(upload_file, 'w', encoding='utf-8') as f:
            json.dump(upload_data, f, indent=2, ensure_ascii=False)
        
        logger.info(f"Successfully uploaded {len(products)} products to {upload_file}")
...
async def run_ai_scrape_task(task_id: str, urls: List[str], max_pages_per_url: int):
    """Run the AI scraping task in the background"""
    start_time = time.time()
    ...
    try:
        ...
        active_tasks[task_id]["current_progress"] = {
            "stage": "ai_analysis",
            "percentage": 10,
            "details": "🧠 AI analyzing page structure...",
            "timestamp": datetime.now().isoformat()
        }
        ...
        active_tasks[task_id].update({
            ...
            "current_progress": {
                "stage": "completed",
                "percentage": 100,
                "details": f"✅ AI scraping completed! Found {len(result.get('products', []))} products with fixed image URLs",
                "timestamp": datetime.now().isoformat()
            }
        })
...
async def save_fixed_results(result: Dict[str, Any], timestamp: str, task_id: str):
    ...
    try:
        ...
        # Save the original (unfixed) version if it doesn't exist
        original_file = os.path.join(logs_dir, f"ai_agent_scrape_{timestamp}.json")
        
        # Save the fixed version
        fixed_file = os.path.join(logs_dir, f"ai_agent_scrape_{timestamp}_FIXED.json")
